 ...
 * 感谢所有测试人员
 * ========= 青龙--配置文件 =========
 * 变量格式: export lxbyhd='encryptData&iv&sessionKey'  多个账号用 @ 或者 换行分割
...
// 跳转的url（https://baiyingmalladmin.lenovo.com/api/v4/user/wxapp/loginWxApp），截取链提交数据内的内容
...
!(async () => {
    let ckArr = await getCks(ckStr, "lxbyhd");
    await tips(ckArr);
...
async function start() {
    ...
    if (ck_status) {
        ...
        await $.wait(2 * 1000);
        /**
        console.log("➡️开始 每日答题");
        await getAnswerStatus();
        await $.wait(2 * 1000);
        */
...
async function gettoken() {
    ...
    if (result.code == 200) {
        ...
        console.log(` 当前获取token: ${result.data.paasToken} ✅ `);
        msg += ` 当前获获取token: ${result.data.paasToken} ✅ `;
...
async function dotask_collectStore(ruId, shopName) {
    ...
    if (result.code == 200) {
        //console.log(result);
        console.log(`\n 店铺 : ${shopName} (${ruId}) --- ${result.data.msg} 🎉 `);
        msg += `\n 店铺 : ${shopName} (${ruId}) --- ${result.data.msg} 🎉 `;
    } else {
        console.log(`\n 关注/取关店铺 : 失败 ❌ 了呢,原因未知！\n ${result} \n`);
        msg += `\n 关注/取关店铺 : 失败 ❌ 了呢,原因未知！\n ${result} \n`;
